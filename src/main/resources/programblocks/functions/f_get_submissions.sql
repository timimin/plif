...
begin
select paper_type(paper_id, title, abstract, text, 'unknown_auth') bulk collect into v_submissions from papers
where paper_id in (select paper_id from submissions where conference_id = c_id);
