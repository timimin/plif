...
THEOREM if_ax == ASSUME NEW p1, NEW p2, NEW p3,
           ...
           <3>3 CASE c1[2][2][e] # {NONE}
                <4>1 matchLocks(c1, c1, e) \subseteq c1[2][2][e] 
                     ...
                     <5>2 CASE c1[2][2][e] \cap UU # {}
                          <6>1 (c1[2][2][e] \ UU)  \subseteq c1[2][2][e] 
                               PROOF OBVIOUS 
                          <6>2 c1 \in ClausesSet
                               PROOF BY ONLY  DEF PoliciesSet                 
                          <6>4 c1[2][2] \in [E1 -> ((SUBSET (U \cup UU)) 
                                                        \ {{}}) \cup {{NONE}}]
                               PROOF BY <6>2, Clause_ASSM1
                          <6>5 DOMAIN c1[2][2] = E1
                               PROOF BY <6>4 
                          <6>6 \/ c1[1] \in UU 
                               \/ /\ c1[1] \notin UU
                                  /\ (UNION Range(c1[2][2])) \cap UU = {}
                               PROOF BY <6>2, Clause_ASSM2
                          <6>7 c1[1] \in UU
                               <7>1  ~(UNION Range(c1[2][2])) \cap UU = {}
                                     PROOF BY <6>5, <5>2 DEF Range
                               <7>2 QED BY <7>1, <6>6
                          <6>8 ((c1[2][2][e] \ UU) \cup {c1[1]}) = c1[2][2][e]                                
                               PROOF BY <5>2, <6>7, UU_ASSM
                          <6>9 QED BY <5>2, <6>8
                     <5>3 QED BY <5>1, <5>2
                <4>2 QED BY <4>1
...
THEOREM ANTYSYMMETRY == \A p1, p2 \in PoliciesSet: 
                           p1 = p2 => comparePol(p1, p2) /\ comparePol(p2, p1)
<1> USE DEF comparePol, compareClause, substMap3Equality, matchLocks
<1>1 TAKE p1 \in PoliciesSet, p2 \in PoliciesSet
<1>2 HAVE p1 = p2
<1>3 \A c2 \in p2 : (\E c1 \in p1 : compareClause (c1, c2))
    <2>1  TAKE c2 \in p2
    <2>2  PICK c1 \in p1: c1 = c2
          PROOF BY <1>2
    <2>3  substMap3Equality(c1,c2)
          PROOF BY <2>2   
    <2>4  \A k \in E0 : \/ c1[2][1][k] = c2[2][1][k]
                        \/ c2[2][1][k] = {}
          PROOF BY <2>2
    <2>5  \A e \in E1 : \/ c1[2][2][e] = {NONE}
                        \/ matchLocks(c1, c2, e) \subseteq c2[2][2][e]
          ...
          <3>2  CASE c1[2][2][e] = {NONE}
                PROOF BY <3>2
          <3>3  CASE c1[2][2][e] # {NONE}
                <4>1 CASE c1[2][2][e] \intersect UU = {}
                     <5>1 c1[2][2][e] \subseteq c2[2][2][e]
                          PROOF BY <2>2
                     <5>2 QED BY <4>1, <5>1, <3>3
                <4>2 CASE c1[2][2][e] \intersect UU # {}
                     <5>1 c2 \in ClausesSet
                          PROOF BY ONLY DEF PoliciesSet
                     <5>2 c2[2][2][e] \intersect UU # {}
                          PROOF BY <2>2, <4>2
                     <5>3 DOMAIN c2[2][2] = E1
                          PROOF BY <5>1, Clause_ASSM1
                     <5>4 ~((UNION Range(c2[2][2])) \cap UU = {})
                          PROOF BY <5>1, <5>2, <5>3, Clause_ASSM2 DEF Range
                     <5>5 c2[1] \in UU
                          PROOF BY <5>1, <5>4, Clause_ASSM2, <2>2
                     <5>6 {c2[1]} = UU
                          PROOF BY <5>5, UU_ASSM
                     <5>7 (c1[2][2][e] \ UU) \union {c2[1]} \subseteq c2[2][2][e]
                          PROOF BY <2>2, <5>6, <4>2
                     <5>8 QED BY <4>2, <5>7, <3>3
                <4>3 QED BY <4>1, <4>2
          <3>4 QED BY <3>2, <3>3
    <2>6  QED BY <2>3, <2>4, <2>5, <2>2
<1>4 \A c1 \in p1 : (\E c2 \in p2 : compareClause (c2, c1))
    <2>1  TAKE c1 \in p1
    <2>2  PICK c2 \in p2: c1 = c2
          PROOF BY <1>2
    <2>3  substMap3Equality(c2,c1)
          PROOF BY <2>2   
    <2>4  \A k \in E0 : \/ c1[2][1][k] = c2[2][1][k]
                        \/ c1[2][1][k] = {}
          PROOF BY <2>2
    <2>5  \A e \in E1 : \/ c2[2][2][e] = {NONE}
                        \/ matchLocks(c2, c1, e) \subseteq c1[2][2][e]
          <3>1  TAKE e \in E1
          <3>2  CASE c2[2][2][e] = {NONE}
                PROOF BY <3>2
          <3>3  CASE c2[2][2][e] # {NONE}
                <4>1 CASE c2[2][2][e] \intersect UU = {}
                     <5>1 c2[2][2][e] \subseteq c1[2][2][e]
                          PROOF BY <2>2
                     <5>2 QED BY <4>1, <5>1, <3>3
                <4>2 CASE c2[2][2][e] \intersect UU # {}
                     <5>1 c1 \in ClausesSet
                          PROOF BY ONLY DEF PoliciesSet
                     <5>2 c1[2][2][e] \intersect UU # {}
                          PROOF BY <2>2, <4>2
                     <5>3 DOMAIN c1[2][2] = E1
                          PROOF BY <5>1, Clause_ASSM1
                     <5>4 ~((UNION Range(c1[2][2])) \cap UU = {})
                          PROOF BY <5>1, <5>2, <5>3, Clause_ASSM2 DEF Range
                     <5>5 c1[1] \in UU
                          PROOF BY <5>1, <5>4, Clause_ASSM2, <2>2
                     <5>6 {c1[1]} = UU
                          PROOF BY <5>5, UU_ASSM
                     <5>7 (c2[2][2][e] \ UU) \union {c1[1]} \subseteq c1[2][2][e]
                          PROOF BY <2>2, <5>6, <4>2
                     <5>8 QED BY <4>2, <5>7, <3>3
                <4>3 QED BY <4>1, <4>2
          <3>4 QED BY <3>2, <3>3
                      ...
                      <5>2 CASE c1[2][2][e] \intersect UU = {}
                           ...
                           <6>2 CASE /\ c1[2][2][e] # {NONE} 
                                ...
                                <7>2 c2[2][2][e] \intersect UU = {}
                                     PROOF BY <6>1, <3>3, UU_ASSM
                                <7>3 c2[2][2][e] \subseteq y1[2][2][e]
                                     PROOF BY <2>6, <5>2, <6>2, <7>2
                                           DEF compareClause, matchLocks   
                                <7>4 QED BY <7>1, <7>3, <6>1, 
                                            <3>3 DEF matchLocks
                           ...
                           <6>4 CASE /\ c1[2][2][e] = {NONE} 
                                ...
                                <7>1 c2[2][2][e] \intersect UU = {}
                                     PROOF BY <6>1, <3>3, UU_ASSM
                                <7>2 c2[2][2][e] \subseteq y1[2][2][e]
                           ...
                           <6>2 CASE /\ c1[2][2][e] # {NONE} 
                                ...
                                <7>3 c2[2][2][e] \subseteq y1[2][2][e]
                                     PROOF BY <2>6, <5>2, <6>2, <7>2
                                           DEF compareClause, matchLocks 
                                <7>4 c2[1] = y1[1]
                                     PROOF BY <2>6, <3>3, <6>1, UU_ASSM
                                           DEF compareClause, substMap3Equality
                                <7>5 QED BY <7>1, <7>3, <7>4, <6>1, 
                                            <3>3 DEF matchLocks
                           <6>3 CASE /\ c1[2][2][e] # {NONE} 
                                     /\ c2[2][2][e] = {NONE}  
                                <7>1 (c1[2][2][e] \ UU) \cup {y1[1]}  
                      ...
                      <5>2 CASE c2[2][2][e] \intersect UU = {}
                           ...
                           <6>2 CASE /\ c1[2][2][e] # {NONE} 
                                ...
                                <7>2 c1[2][2][e] \intersect UU = {}
                                     PROOF BY <6>1, <3>3, UU_ASSM
                                <7>3 c1[2][2][e] \subseteq y1[2][2][e]
                           ...
                           <6>4 CASE /\ c2[2][2][e] = {NONE} 
                                ...
                                <7>1 c1[2][2][e] \intersect UU = {}
                                     PROOF BY <6>1, <3>3, UU_ASSM
                                <7>2 c1[2][2][e] \subseteq y1[2][2][e]
                 ...
                 <4>2 CASE substMap3Equality(c1,c2)               
                      ...
                      <5>3 CASE c1[2][2][e] \intersect UU # {}
                           ...
                           <6>4 CASE /\ c1[2][2][e] # {NONE} 
                                ...
                                <7>6 {c2[1]} = UU
                                     PROOF BY  <7>5, Clause_ASSM2, UU_ASSM
                                           DEF PoliciesSet                                
...
\* Last modified Tue Apr 05 19:26:52 MSK 2022 by user-sc
