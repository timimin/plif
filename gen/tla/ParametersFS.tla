...
P_ADD_PAPER_p_ABSR(x) == [loc|->"mem", offs |->2, policy |-> any_caller(x)]
P_ADD_PAPER_p_T(x) == [loc|->"mem", offs |->3, policy |-> any_caller(x)]
P_ADD_PAPER_p_AUTH(x) == [loc|->"mem", offs |->4, policy |-> any_caller(x)]

P_ALLOCATE_p_ID(x) == [loc|->"mem", offs |->0, policy |-> any_caller(x)]
P_ALLOCATE_p_S_ID(x) == [loc|->"mem", offs |->1, policy |-> any_caller(x)]
P_ALLOCATE_p_SEC_ID(x) == [loc|->"mem", offs |->2, policy |-> any_caller(x)]
P_ALLOCATE_p_ALLOC_DATE(x) == [loc|->"mem", offs |->3, policy |-> any_caller(x)]
P_ALLOCATE_e_PAPER_NOT_ACCEPTED(x) == [loc|->"mem", offs |->4, policy |-> min]
P_ALLOCATE_v_V_P_ID(x) == [loc|->"mem", offs |->5, policy |-> min]
P_ALLOCATE_v_V_IS_ACC(x) == [loc|->"mem", offs |->6, policy |-> min]
